    ...
    <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'DM Sans',sans-serif", background: C.bgAlt }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .dot{width:7px;height:7px;border-radius:50%;background:#B87333;animation:pulse 1.2s ease infinite;display:inline-block;margin:0 2px}
        .dot2{animation-delay:.2s}
        .dot3{animation-delay:.4s}
        .send-btn{background:#3E2723;color:#FEFEF8;border:none;border-radius:10px;padding:10px 16px;cursor:pointer;font-family:inherit;transition:all .2s;display:flex;align-items:center;justify-content:center;flex-shrink:0}
        .send-btn:hover{background:#6D4C41}
        .send-btn:disabled{background:#D4C4B0;cursor:not-allowed}
        .sug{background:#FEFEF8;border:1px solid #D4C4B0;border-radius:10px;padding:10px 14px;font-size:.8rem;color:#6D4C41;cursor:pointer;transition:all .2s;text-align:left;font-family:inherit;line-height:1.4}
        .sug:hover{border-color:#B87333;color:#3E2723;background:rgba(184,115,51,.04)}
        .sh{scrollbar-width:thin;scrollbar-color:#D4C4B0 transparent}
        .sh::-webkit-scrollbar{width:4px}
        .sh::-webkit-scrollbar-thumb{background:#D4C4B0;border-radius:4px}
      `}</style>
      ...
      <main style={{ marginLeft: 220, flex: 1, display: 'flex', flexDirection: 'column', height: '100vh', maxWidth: 'calc(100vw - 220px)' }}>
          ...
          {uploads.filter(u => u.status === 'completed').length > 0 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <select
                ...
                onChange={e => { setSelectedUpload(e.target.value); setMessages([]) }}
          ...
          {messages.length === 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 32 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'rgba(184,115,51,0.1)', border: '2px solid rgba(184,115,51,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', color: C.copper }}>
                  {Icons.chat}
                </div>
                ...
                <p style={{ fontSize: '.875rem', color: C.gray, maxWidth: 400, lineHeight: 1.6 }}>
                  {selectedUpload ? 'Your validated facility data is ready.' : 'No completed uploads found. Upload a file first.'}
                </p>
              </div>
              {selectedUpload && (
          ...
          {messages.map(msg => (
            <div key={msg.id} className="msg-appear" style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, flexDirection: msg.role === 'user' ? 'row-reverse' : 'row' }}>
                <div style={{ width: 30, height: 30, borderRadius: '50%', background: msg.role === 'user' ? 'linear-gradient(135deg,#B87333,#D4AF37)' : C.bgAlt, border: '1px solid ' + C.taupe, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '.75rem', fontWeight: 600, color: msg.role === 'user' ? '#FEFEF8' : C.copper, flexShrink: 0 }}>
                  {msg.role === 'user' ? user.name.charAt(0).toUpperCase() : 'AI'}
                </div>
                <div style={{ maxWidth: 560, padding: '13px 17px', borderRadius: msg.role === 'user' ? '16px 4px 16px 16px' : '4px 16px 16px 16px', background: msg.role === 'user' ? C.brown : C.bg, color: msg.role === 'user' ? '#FEFEF8' : C.brown, border: msg.role === 'assistant' ? '1px solid ' + C.taupe : 'none', fontSize: '.9rem', lineHeight: 1.75 }}>
                  {msg.loading ? (
              ...
              {msg.sources && msg.sources.length > 0 && (
                <div style={{ maxWidth: 560, marginLeft: 40, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <div style={{ fontSize: '.7rem', color: C.gray, letterSpacing: '.08em', textTransform: 'uppercase' }}>
                    Sources - {(((msg.confidence || 0) * 100).toFixed(0))}% confidence
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    {msg.sources.slice(0, 4).map((src, i) => (
                      <div key={i} style={{ padding: '8px 12px', background: C.bg, border: '1px solid ' + C.taupe, borderRadius: 8, fontSize: '.75rem' }}>
                        ...
                        <div style={{ color: C.gray, marginTop: 2 }}>{src.period} - {(src.similarity * 100).toFixed(0)}% match</div>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        <div style={{ padding: '16px 32px 24px', background: C.bg, borderTop: '1px solid ' + C.taupe, flexShrink: 0 }}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-end', background: C.bgAlt, border: '1.5px solid ' + C.taupe, borderRadius: 14, padding: '12px 16px' }}>
            <textarea
              ref={inputRef}
              rows={1}
              ...
              style={{ resize: 'none', outline: 'none', border: 'none', background: 'transparent', fontFamily: 'inherit', fontSize: '.9rem', color: C.brown, width: '100%', lineHeight: 1.5, maxHeight: 120 }}
            />
