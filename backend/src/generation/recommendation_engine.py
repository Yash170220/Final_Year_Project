    ...
    def _generate_ai_suggestions(
        ...
        try:
            response = self.groq.chat.completions.create(
                model=self.model,
                messages=[
                    # FIX: Added system prompt — was missing, causing vague/generic suggestions
                    {"role": "system", "content": RECOMMENDATION_SYSTEM_PROMPT},
                    {"role": "user", "content": prompt},
                ],
                temperature=0.2,   # FIX: was 0.4 — lower = more consistent, specific output
                max_tokens=400,    # FIX: was 600 — 3 recommendations don't need 600 tokens
            )
